...
# --- 1. DATA-INNGANG (Ingen filopplasting = Ingen Axios Error) ---
st.subheader("1. Lim inn måledata her")
raw_data = st.text_area("Lim inn kolonner fra CSV/Excel her (Tid og Nivå)", height=150, placeholder="0,10.5\n1,11.2\n2,12.1...")

df = None
if raw_data:
    try:
        df = pd.read_csv(io.StringIO(raw_data), sep=None, engine='python', decimal=',', names=['Tid', 'Nivå'])
    except:
        st.error("Feil format! Sjekk at du har limt inn to kolonner.")

# Knapp for å teste hvis man ikke har data for hånd
...
if df is not None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        A = st.slider("Δy (Gain)", 0.0, float(max(A_est*2.5, 1.0)), float(A_est))
        T = st.slider("T (Tidskonstant)", 0.1, 200.0, 20.0)
    with c2:
        L = st.slider("L (Dødtid)", 0.0, float(tid_data[-1]/2), 5.0)
        y0 = st.slider("y0 (Start)", float(y0_est-10), float(y0_est+10), float(y0_est))
    ...
    ax.plot(tid_data, y_model, "r-", linewidth=2.5, label="Modell")
    ...
    st.info("💡 Tips for mobil: Åpne CSV-fila di, velg 'Marker alt', 'Kopier', og lim inn i feltet over.")
